...
import { deleteGameGlobal } from "@/app/actions";
import {
  Dialog,
  DialogClose,
  DialogContent,
...
export function DeleteGameGlobalDialog({
  gameId,
  gameName,
}: {
  gameId: string;
  ...
  return (
    <Dialog>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Supprimer un jeu global</DialogTitle>
          <DialogDescription>
            Cette action supprimera le jeu {gameName}. Elle est impossible si le
            jeu est deja utilise dans des tables.
          </DialogDescription>
        </DialogHeader>
        <form action={deleteGameGlobal} className="mt-4 space-y-3">
