...
export function DeletePlayerDialog({
  ...
  const isMatch = useMemo(
    () => confirmText.trim().toLowerCase() === playerName.trim().toLowerCase(),
  ...
  return (
    <Dialog>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Supprimer un joueur</DialogTitle>
          <DialogDescription>
            Cette action supprimera {playerName} de la session et ses
            affectations de table.
          </DialogDescription>
        </DialogHeader>
        <form action={deletePlayer} className="mt-4 space-y-3">
          <input type="hidden" name="sessionId" value={sessionId} />
          <input type="hidden" name="playerId" value={playerId} />
          <div className="space-y-2">
            <label className="text-xs font-medium text-muted-foreground">
              Tapez le nom du joueur pour confirmer
            </label>
            <input
              value={confirmText}
