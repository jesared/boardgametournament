...
export function DeleteSessionDialog({
  ...
  const isMatch = useMemo(
    () => confirmText.trim().toLowerCase() === sessionName.trim().toLowerCase(),
    [confirmText, sessionName],
  );
    ...
    <Dialog>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Cette action supprimera la session {sessionName} ainsi que les
            joueurs, manches, tables et resultats associes.
          </DialogDescription>
        </DialogHeader>
        <form action={deleteSession} className="mt-4 space-y-3">
          <input type="hidden" name="sessionId" value={sessionId} />
          <div className="space-y-2">
            <label className="text-xs font-medium text-muted-foreground">
              Tapez le nom de la session pour confirmer
            </label>
            <input
              value={confirmText}
